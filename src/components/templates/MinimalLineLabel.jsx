...
import { calcPercentDV, perServingWithUnit } from '../../utils'
...
export default function MinimalLineLabel({ data, servingGrams }) {
  return (
    <>
      ...
      <div className="minimal-serving">Per serving ({data.servingSize || '100g'})</div>
      ...
      {ROWS.map(({ name, field, unit, dvKey, sub }) => (
        <div key={field} className={`minimal-row${sub ? ' sub' : ''}`}>
          ...
          <span className="minimal-val">{perServingWithUnit(data[field], servingGrams, unit)}</span>
