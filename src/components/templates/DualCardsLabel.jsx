...
export default function DualCardsLabel({ data, servingGrams }) {
  return (
    <>
      ...
      <div className="cards-serving">Serving size {data.servingSize || '100g'}</div>
        ...
        {ROWS.map(({ name, field, unit, dvKey }) => (
          <div key={field} className="nutrient-card">
            ...
            <div className="nutrient-card-vals">
              <div><span className="nc-label">100g</span>{fmtWithUnit(data[field], unit)}</div>
              <div><span className="nc-label">Serving</span>{perServingWithUnit(data[field], servingGrams, unit)}</div>
            </div>
